...
template <typename Graph> struct GraphTraits {
  ...
  static SuccIterator succ_end(Graph &owner, const NodeId &node) { return GraphType::succ_end(owner, node); }
};
...
template <bool Backward = false, typename Graph, typename Visitor, typename PostVisitor>
void visit_dfs_conditional(Graph &&graph, const typename GraphTraits<Graph>::NodeId &entry, Visitor &&visitor,
  ...
  *worklist_inserter = entry;
  ...
  while (!worklist.empty()) {
    ...
    if (visitor_res) {
      auto worklist_pusher = [&worklist_inserter, &visited](typename Traits::NodeId node) {
      ...
      if (Backward) {
        std::for_each(Traits::pred_begin(graph, tail), Traits::pred_end(graph, tail), worklist_pusher);
      } else {
        std::for_each(Traits::succ_begin(graph, tail), Traits::succ_end(graph, tail), worklist_pusher);
      }
    }
...
template <bool Backward = false, typename Graph, typename Visitor>
void visit_dfs_conditional(Graph &&graph, const typename GraphTraits<Graph>::NodeId &entry,
                           Visitor &&visitor) {
  visit_dfs_conditional<Backward>(std::forward<Graph>(graph), entry, std::forward<Visitor>(visitor),
